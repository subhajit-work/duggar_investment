...
export class AddCountryPage implements OnInit, OnDestroy {
  form: any;
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      // this.getlist('country_getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'country/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'country'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              iso_code : res.return_data.iso_code,
              tel_prefix: res.return_data.tel_prefix,
              flag: res.return_data.flag,
...
// ======================= form submit start ===================
  ...
  onSubmit(form:NgForm){
    ...
    if(this.fileVal) {
    // normal file upload
      fd.append("flag", this.fileVal, this.fileVal.name);
    }else{
      fd.append("flag", '');
    }
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        iso_code : this.allEditData.return_data.iso_code,
        tel_prefix: this.allEditData.return_data.tel_prefix,
        flag: this.allEditData.return_data.flag,
...
// reload page end
  ...
  // Normal file upload
    ...
    normalFileUpload(event) {
      ...
      this.model.flag =  event.target.files[0].name;
    }
    fileCross(_item1){
      this.model.flag = '';
      this.model.flag2 = '';
