...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('location_getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'location/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'location'
      }
  ...
  init(){
    if( this.parms_action_name == 'edit'){
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              code: res.return_data.code,
              address_line1 : res.return_data.address_line1,
              address_line2 : res.return_data.address_line2,
              country_id :  JSON.parse(res.return_data.country_id),
              state : res.return_data.state,
              city : res.return_data.city,
              pin : res.return_data.pin,
              lead_manager_id: JSON.parse(res.return_data.lead_manager_id),
              enable : parseInt(res.return_data.status),
              
            };

            this.model.branch_admin_model = [];
            if(res.return_data.location_admin){
              res.return_data.location_admin.forEach(element => {
                this.model.branch_admin_model.push(element.id);
              });
            }
            // edit data
      ...
      // status
      /* console.log('status get format >', this.model.enable);
    ...
    this.plt.ready().then(() => {
      ...
      this.getListSubscribe = this.commonUtils.getlistCommon(_getlistUrl).subscribe(
        resData => {
          ...
          this.countrysList = resData.country;
          this.employeeList = resData.employee;
        },
        errRes => {
          this.selectLoading = false;
        }
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        code: this.allEditData.return_data.code,
        address_line1 : this.allEditData.return_data.address_line1,
        address_line2 : this.allEditData.return_data.address_line2,
        country_id :  JSON.parse(this.allEditData.return_data.country_id),
        state : this.allEditData.return_data.state,
        city : this.allEditData.return_data.city,
        pin : this.allEditData.return_data.pin,
        lead_manager_id: JSON.parse(this.allEditData.return_data.lead_manager_id),
        enable : parseInt(this.allEditData.return_data.status),
        
      };

      this.model.branch_admin_model = [];
      if(this.allEditData.return_data.location_admin){
        this.allEditData.return_data.location_admin.forEach(element => {
          this.model.branch_admin_model.push(element.id);
        });
      }
