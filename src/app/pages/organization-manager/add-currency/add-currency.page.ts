...
@Component({
  ...
  templateUrl: './add-currency.page.html',
  styleUrls: ['./add-currency.page.scss'],
})
 
export class AddCurrencyPage implements OnInit, OnDestroy {
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      // this.getlist('currency_getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'currency/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'currency'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              iso_prefix: res.return_data.iso_prefix,
              symbol: res.return_data.symbol,
...
onSubmit(form:NgForm){
  ...
  if(this.fileVal) {
  // normal file upload
    fd.append("icon", this.fileVal, this.fileVal.name);
  }else{
    fd.append("icon",'');
  }
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        iso_prefix: this.allEditData.return_data.iso_prefix,
        symbol: this.allEditData.return_data.symbol,
...
// Normal file upload
  ...
  normalFileUpload(event) {
    ...
    this.model.icon =  event.target.files[0].name;
  }
  fileCross(_item1){
    this.model.icon = '';
    this.model.icon2 = '';
