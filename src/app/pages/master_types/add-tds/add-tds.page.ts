...
@Component({
  ...
  templateUrl: './add-tds.page.html',
  styleUrls: ['./add-tds.page.scss'],
})

export class AddTdsPage implements OnInit, OnDestroy {
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('tds_getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'tds/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'tds'
      }
      ...
      this.editDataSubscribe = this.http.get(this.form_api).subscribe(
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              interest_tds : res.return_data.interest_tds,
              brokerage_tds : res.return_data.brokerage_tds,
...
//----------- reload page start ------------
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        interest_tds : this.allEditData.return_data.interest_tds,
        brokerage_tds : this.allEditData.return_data.brokerage_tds,
