...
/* tslint:disable */ 
@Component({
  selector: 'app-add-fiscalyear',
  templateUrl: './add-fiscalyear.page.html',
  styleUrls: ['./add-fiscalyear.page.scss'],
})

export class AddFiscalyearPage implements OnInit, OnDestroy {
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      // this.getlist('fiscalyear_getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'fiscalyear/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'fiscalyear'
      }
  ...
  init(){
      ...
      this.editDataSubscribe = this.http.get(this.form_api).subscribe(
        (res:any) => {
          ...
          if(res.return_status > 0){
            ...
            if(res.return_data.status == '1' || res.return_data.status == 'true'){
              this.model.enable = res.return_data.status;
            }else{
              this.model.enable = '';
            }
    ...
    this.model.end_date =  this.setStartdate;
    // console.log('Date >>>>>>>',this.model.dob);
  }
  // ---------- init end ----------
      ...
      (response:any) => {
        ...
        if(response.return_status > 0){
          ...
          if(this.clickButtonTypeCheck == 'Save'){
            ...
            this.router.navigate(['/fiscalyear-list']);
...
//----------- reload page start ------------
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      if(this.allEditData.return_data.status == '1' || this.allEditData.return_data.status == 'true'){
        this.model.enable = this.allEditData.return_data.status;
      }else{
        this.model.enable = '';
      }
