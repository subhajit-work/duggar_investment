...
/* tslint:disable */ 
@Component({
  selector: 'app-add-graceday',
  templateUrl: './add-graceday.page.html',
  styleUrls: ['./add-graceday.page.scss'],
})
 
export class AddGracedayPage implements OnInit, OnDestroy {
  constructor(
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      if( this.parms_action_name == 'edit'){
        ...
        this.form_api = 'graceday/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'graceday'
      }
      ...
      this.editDataSubscribe = this.http.get(this.form_api).subscribe(
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              start_day: res.return_data.start_day,
              end_day: res.return_data.end_day,
              increase_day: res.return_data.increase_day,
...
//----------- reload page start ------------
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        start_day: this.allEditData.return_data.start_day,
        end_day: this.allEditData.return_data.end_day,
        increase_day: this.allEditData.return_data.increase_day,
