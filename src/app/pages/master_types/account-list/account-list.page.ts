...
export class AccountListPage implements OnInit, OnDestroy {
  ...
  showClickEl;
  toggleMobileSearch;
  change_mainAccountUrl;
  private mainAccountSubscribe: Subscription;
  selectmainAccount;
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.onHeaderData('accountbranch_headers');

        // table list data url name
        this.listing_url = 'accountbranch';

        // change mainAccount Url
        this.change_mainAccountUrl = 'accountbranch_changemain/';
  
        // getlist data url name
        // this.getlist('phonetype_getlist');
    ...
    onListData(_list_url, _displayRecord, _page, _apiParms, _search, _cherecterSearch, _orderBy, _order, _advSearchParms, _identifire) {
      this.plt.ready().then(() => {
        ...
        this.itemsSubscribe = this.commonUtils.fetchList(_list_url, _displayRecord, _page, _apiParms,  _search, _cherecterSearch, _orderBy, _order, _advSearchParms, _identifire).subscribe(
          ...
          // this.selectmainAccount
          this.fetchItems.forEach((element, index) => {
            if(element.main == '1'){
              let ind = parseInt(index) + 1;
              this.selectmainAccount = JSON.stringify(index);
            }
          });
    ...
    // main account select
    changeMainAccount(_itemId){
      console.log('main account select >>>', _itemId);

      this.mainAccountSubscribe = this.http.get(this.change_mainAccountUrl+_itemId).subscribe(
        (res:any) => {
          console.log("main account  data  res >", res.return_data);
          if(res.return_status > 0){
            this.commonUtils.presentToast('success', res.return_message);
          }
        },
    ...
    // ----------- destroy subscription start ---------
      ngOnDestroy() {
        ...
        if(this.mainAccountSubscribe !== undefined){
          this.mainAccountSubscribe.unsubscribe();
        }
