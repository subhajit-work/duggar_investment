...
/* tslint:disable */ 
@Component({
  selector: 'app-graceday-list',
  templateUrl: './graceday-list.page.html',
  styleUrls: ['./graceday-list.page.scss'],
})
// End here


export class GracedayListPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.onHeaderData('graceday_headers');

        // table list data url name
        this.listing_url = 'graceday';
  
        // getlist data url name
        this.getlist('graceday_getlist');
        ...
        this.plt.ready().then(() => {
          ...
          this.getListSubscribe = this.commonUtils.getlistCommon(_getlistUrl).subscribe(
            resData => {
              ...
              /* this.model = {
                start_day: resData.start_day,
                end_day: resData.end_day,
                increase_day: resData.increase_day,
                enable: resData.status
              } */
            },
            errRes => {
              this.selectLoading = false;
