...
export class InvoiceListPage implements OnInit, OnDestroy {
  ...
  constructor(
        ...
        if(_identifire == 'Add Interest' && _type == 'multiple'){
          ...
          for (let i = 0 ; i < _selectItem.length; i++) {
            if( !(firstEl.lender_id == _selectItem[i].lender_id) || !(firstEl.brrower_id == _selectItem[i].brrower_id)){
              ...
              break;
            }else if(!(_selectItem[i].transaction_status == '0') || (_selectItem[i].payment_mode == 'cheque' && _selectItem[i].transaction_status_name == 'Running')){
    ...
    onExport(_identifier, _items, _item){
      ...
      if(_identifier == 'original'){
        this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
          this.export_url = this.main_url+'/transaction_invoice/'+_item.id+'?token='+res.token+'&session='+res.session+'&master='+res.master+'&identifier=original';
        });
        window.open(this.export_url);

      }else if(_identifier == 'duplicate'){
        this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
          this.export_url = this.main_url+'/transaction_invoice/'+_item.id+'?token='+res.token+'&session='+res.session+'&master='+res.master+'&identifier=duplicate';
        });
        window.open(this.export_url);
        ...
        this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
          ...
          this.export_url = this.main_url+'/transaction_invoice?per_page='+this.displayRecord+'&page='+this.pageNo+'&token='+res.token+'&session='+res.session+'&master='+res.master;
          });
        window.open(this.export_url);
      }else{
        this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
